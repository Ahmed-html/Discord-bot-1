...
module.exports.run = async (bot, message, args) => {
	...
	if (ms(Timer) > 2147483647)
		return message.reply(
			"Enter a number lower than 2147483647, idiot."
		);
	if (ms(Timer) < 1) return message.reply("Enter an number larger that 1, idiot");

	if (!args[0]) {
		return message.channel.send(
			":x: " + '| Enter a time period followed by "s or m or h, idiot"'
		);
	}
