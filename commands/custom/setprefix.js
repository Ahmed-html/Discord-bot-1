...
module.exports.run = async (client, message, args) => {
	...
	const embedmissing = new Discord.MessageEmbed()
		.setDescription(`:x: Idiot, type the prefix you want to set!`)
		.setColor("RED");

	const embedtoolong = new Discord.MessageEmbed()
		.setDescription(
			`:x: Idiot, length shouldn't be longer than 3 letters`
		)
		.setColor("RED");
	const embedsame = new Discord.MessageEmbed()
		.setDescription(`:x: Idiot, prefix is same to current's`)
		.setColor("RED");
	...
	const calcelembed = new Discord.MessageEmbed()
		.setDescription(
			`:x: Time's up! Setting prefix's action cancelled! So not halal mode`
		)
		.setColor("RED");
	...
	await message.channel
		...
		.then(async collected => {
			...
			const doneembed = new Discord.MessageEmbed()
				.setDescription(
					`:white_check_mark: Prefix has been changed to: \`${client.settings.get(
						message.guild.id,
						"prefix"
					)}\``
				)
