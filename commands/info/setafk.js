module.exports.run = (client, message, args) => {
	...
	if (setStatus[1] === "afk") {
		...
		message.channel.send(":white_check_mark: Your status has been set to afk!");
	} else if (setStatus[1] === "notafk") {
		client.user.setAFK(false);
		message.channel.send(`Welcome back ${message.author}`);
	} else if (!setStatus[1] || setStatus[1] === undefined) {
		message.channel.send("You did not choose `afk` or `notafk` as current status, idiot");
	} else {
		message.channel.send("You did not choose `afk` or `notafk` as current status, idiot");
	}
};
