...
module.exports.run = (client, message, args) => {
	const dfgrdgdfgdf = new Discord.MessageEmbed()
		.setDescription(`:white_check_mark: **Lockdown lifted**`)
		.setColor("GREEN");
	const notice3 = new Discord.MessageEmbed()
		.setDescription(
			`:x: I don't have permission to manage channel! So not halal mode.`
		)
		.setColor("RED");
	...
	const ddd = new Discord.MessageEmbed()
		.setDescription(
			`:x: **You must set a duration for the lockdown in either hours, minutes or seconds, idiot.**`
		)
		.setColor("RED");
		...
		message.channel
			...
			.then(() => {
				const bsuembed = new Discord.MessageEmbed()
					.setDescription(
						`:white_check_mark: Locked the channel down for **${ms(ms(time), {
							long: true
						})}**`
					)
					.setColor("BLUE");
