...
module.exports.run = async (client, msg, args) => {
	const notice3 = new Discord.MessageEmbed()
		.setDescription(`:x: I don't have permission to kick people! So not halal mode.`)
		.setColor("RED");
	...
	const embed6 = new Discord.MessageEmbed()
		.setDescription(
			`:x: ${msg.author.username}, Missing Permission! So not halal mode.`
		)
		.setColor("RED");
	...
	const dsfdsfsdf = new Discord.MessageEmbed()
		.setDescription(
			`:x: Access Denied, that member has roles higher or equal to you, idiot.`
		)
		.setColor("RED");
	...
	const notice2 = new Discord.MessageEmbed()
		.setDescription(`:x: You cannot kick yourself, idiot`)
		.setColor("RED");
	...
	const sdfdfsdfsdfdfs = new Discord.MessageEmbed()
		.setDescription(
			`:x: An error occurred when banning that member, do something right in your life.`
		)
		.setColor("RED");
