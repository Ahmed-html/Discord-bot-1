...
module.exports.run = async (client, message, args) => {
	...
	const missingn = new Discord.MessageEmbed()
		.setDescription(
			`:x: Use: **\`${prefixesdatabase.prefix}clear <1 - 100>\`**`
		)
		.setColor("RED");
	...
	const embedgg = new Discord.MessageEmbed()
		.setColor("RED")
		.setDescription(
			`:x: You can only clear 1 - 100 message(s) each time, idiot.`
		);
	...
	const kntlembed = new Discord.MessageEmbed()
		.setColor("BLUE")
		.setDescription(`:white_check_mark: Cleared **${args[0]}** Message here`);
